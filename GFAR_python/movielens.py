...
'''
Download the dataset from https://grouplens.org/datasets/movielens/1m/ 
This script Creates 5 random splits and train validation and test sets from 'ratings.dat' file!
'''
...
raw_data = pd.read_csv(os.path.join(DATA_DIR, 'ratings.dat'), header=0, sep='::',
...
# NOTE that now the user and item ids are converted to incremental integer ids. So, they are not the same as original
# dataset
...
for i in range(1, 6):
    ...
    print("seed : " + str(seed))
    # 60% train 20% validation and 20% test set.
